...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        description='Creates a new mobile_catkin_modules_build_development_tools package based on catkin and gradle. \n')
...
def add_tasks_to_cmake_setup(tasks):
    ...
    with open(cmakelists_txt_path, 'r') as cmakelists_txt:
        ...
        result = re.search('^catkin_mobile_catkin_modules_build_development_tools_setup\(.*\)', old_contents, re.MULTILINE)
        if result is None:
            console.pretty_println("\nCouldn't find a catkin_mobile_catkin_modules_build_development_tools_setup entry in the CMakeLists.txt - not adding tasks.")
            return
        mobile_catkin_modules_build_development_tools_setup_string = result.group(0)
        gradle_tasks = set([])
        if mobile_catkin_modules_build_development_tools_setup_string.find("publish") == -1:
            gradle_tasks.add("publish")
        if mobile_catkin_modules_build_development_tools_setup_string.find("installDist") == -1:
        ...
        old_text = mobile_catkin_modules_build_development_tools_setup_string
        new_text = 'catkin_mobile_catkin_modules_build_development_tools_setup(' + ' '.join(gradle_tasks) + ')'
...
def create_mobile_catkin_modules_build_development_tools_project_common(args, template_directory):
    project_name = args.name[0]
    console.pretty_println("\nCreating mobile_catkin_modules_build_development_tools project ", console.bold)
...
def create_mobile_catkin_modules_build_development_tools_project():
    args = parse_arguments()
    project_name = args.name[0]
    author = args.author
    create_mobile_catkin_modules_build_development_tools_project_common(args, 'mobile_catkin_modules_build_development_tools_project')
    create_talker_listener_classes(project_name, 'mobile_catkin_modules_build_development_tools_project', author)
    add_tasks_to_cmake_setup(['installDist', 'publish'])


def create_mobile_catkin_modules_build_development_tools_library_project():
    args = parse_arguments()
    project_name = args.name[0]
    create_mobile_catkin_modules_build_development_tools_project_common(args, 'mobile_catkin_modules_build_development_tools_library_project')
    create_dummy_java_class(project_name)
    add_tasks_to_cmake_setup(['publish'])


def create_mobile_catkin_modules_build_development_tools_msg_project():
    args = parse_arguments()
    project_name = args.name[0]
    create_mobile_catkin_modules_build_development_tools_project_common(args, 'mobile_catkin_modules_build_development_tools_msg_project')
